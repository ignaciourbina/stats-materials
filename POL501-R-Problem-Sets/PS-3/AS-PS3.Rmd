...
kable(data.frame(crimesafe_CI_z_95))
```
...
kable(data.frame(crimesafe_CI_z_99))
...
df_dem_and_rep <- data.frame(df_dems, df_rep)
...
crimesafe_CI_z_95_dems <- confidence_interval(data = df_dem_and_rep, var_name = 'df_dems', confidence_level = 0.95, method = "z")
...
crimesafe_CI_z_95_reps <- confidence_interval(data = df_dem_and_rep, var_name = 'df_rep', confidence_level = 0.95, method = "z")
...
```{r q2-c, echo=TRUE}
# Question 2.c: Using the subsample for Republicans, compute the p-value with the null hypothesis
# that the true proportion for Republicans differs from the sample proportion for Democrats.
