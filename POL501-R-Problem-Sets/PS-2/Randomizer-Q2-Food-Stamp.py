...
def generate_random_data():
    ...
    benefit_per_person = np.random.randint(
        MIN_BENEFIT_PER_PERSON, MAX_BENEFIT_PER_PERSON + 1
    )
    ...
    participant_numbers = np.arange(
        MIN_PARTICIPANTS, MAX_PARTICIPANTS + 1, PARTICIPANT_STEP
    )
    # Randomly select participant levels
    participant_values = np.sort(
        np.random.choice(participant_numbers, num_levels, replace=False)
    )
    ...
    pmf_table = pd.DataFrame(
        {"Participants (P)": participant_values, "Probability P(P=p)": probabilities}
    )
    ...
    pmf_table = pd.concat(
        [
            pd.DataFrame(
                {
                    "Participants (P)": [f"P < {participant_values.min()}"],
                    "Probability P(P=p)": [0],
                }
            ),
            pmf_table,
            pd.DataFrame(
                {
                    "Participants (P)": [f"P > {participant_values.max()}"],
                    "Probability P(P=p)": [0],
                }
            ),
        ],
