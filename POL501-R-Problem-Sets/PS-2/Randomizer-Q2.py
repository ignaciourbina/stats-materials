...
def generate_random_data():
    ...
    attendee_values = np.sort(
        np.random.choice(attendee_numbers, num_levels, replace=False)
    )
    ...
    pmf_table = pd.DataFrame(
        {"Attendees (A)": attendee_values, "Probability P(A=a)": probabilities}
    )
    ...
    pmf_table = pd.concat(
        [
            pd.DataFrame(
                {
                    "Attendees (A)": [f"A < {attendee_values.min()}"],
                    "Probability P(A=a)": [0],
                }
            ),
            pmf_table,
            pd.DataFrame(
                {
                    "Attendees (A)": [f"A > {attendee_values.max()}"],
                    "Probability P(A=a)": [0],
                }
            ),
        ],
